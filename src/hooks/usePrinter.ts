...
export function usePrinter({ name = '_blank', height = 400, width = 600 }: IusePrinter) {
    const print = (detail: { was: string, now: string, item: string, vendor: string }) => {
        ...
        if (mywindow) {
            ...
            // )
            mywindow.document.write(
                `<div style="display:flex; justify-content:center; align-items:center; height:100px; ${currentDiscount(theColorIdx)}"><h2 style=" text-align:center">EVERY SALE BANGS A NAIL</h2></div></div>`
            )
