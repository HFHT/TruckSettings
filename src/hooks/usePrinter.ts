    ...
    const print = (detail: { was: string, now: string, item: string, vendor: string }) => {
        ...
        if (mywindow) {
            ...
            mywindow.document.write(
                ...
                <img height="70" style="margin-left:1rem;"\
                src="${import.meta.env.VITE_STORAGEIMAGEURL}WAS.png"/>\
                </td><td><h1 style="font-size:3.8rem; margin-left:4rem; text-decoration-line: underline;">${detail.was}</h1></td></tr>`
            )
            mywindow.document.write(
                `<tr style="display:flex; align-items:center "><td>\
                <img height="70" style="margin-left:1rem;"\
                src="${import.meta.env.VITE_STORAGEIMAGEURL}NOW.png"/>\
                </td><td><h1 style="font-size:3.8rem; margin-left:4rem; text-decoration-line: underline;">${detail.now}</h1></td></tr>`
            )
            mywindow.document.write(
                `<tr style="display:flex; align-items:center "><td>\
                <img height="70" style="margin-left:1rem;"\
                src="${import.meta.env.VITE_STORAGEIMAGEURL}ITEM.png"/>\
                </td><td><h1 style="font-size:3.8rem; margin-left:3rem; text-decoration-line: underline;">${detail.item}</h1></td></tr>\
                </table>`
            )
            // mywindow.document.write(
            //     `<img width="480"\
            ...
            // )
            mywindow.document.write(
                `<div style="display:flex; justify-content:center; align-items:center; height:80px; ${currentDiscount(theColorIdx)}"><h2 style=" text-align:center">EVERY SALE BANGS A NAIL</h2></div></div>`
            )
